...
function navMenu:show(params)
	...
	self.hide = function()
		GC.LISTMENU_HOLDER.menu.isVisible = false
		GC.LISTMENU_HOLDER.tween = transition.to(GC.LISTMENU_HOLDER.list,{x = minX, time = ANIMATION_TIME_MS, onComplete = hideDone})
		GC.LISTMENU_HOLDER.tween2 = transition.to(GC.LISTMENU_HOLDER.shadow,{alpha = 0, time = ANIMATION_TIME_MS})
		GC.TOOLS_OVERLAY = false
		Runtime:removeEventListener( "key", onKeyEventTool1 )
	end
...
local function onKeyEventTool1( event )
   local phase = event.phase
   local keyName = event.keyName
   local handled = false

   if ( "back" == keyName and phase == "up" ) and GC.TOOLS_OVERLAY == true then
      --handleBack()
	  print(" %%%%%%%%%%%%%%%%%% CHECKING AND CATCHING A BACK PRESS WHEN THE MENU SECTION IS AVAILABLE!")
		GC.LISTMENU_HOLDER.hide()
   return true
   end
end

--add the key callback
Runtime:addEventListener( "key", onKeyEventTool1 )
