...
function alertBox:show(params)
	...
	GC.LISTWIDGET_HOLDER = self
	GC.GOT_LIST = true -- NOTE: This was uncommented on 04/29 but not on 05/07???
	Log("CALLING OUT THE SMALL LIST HERE")
	...
	function self:dismiss()
		...
		if (self) then
			...
			self:removeSelf()
			self = nil
			GC.GOT_LIST = false -- NOTE: This disappeared between 04/29 and 05/07???
			_G.alert = nil
		end
	end
	
local function onKeyEventList1( event )
   local phase = event.phase
   local keyName = event.keyName
   local handled = false

   if ( "back" == keyName and phase == "up" ) and GC.GOT_LIST == true then
      --handleBack()
	  GC.BACKPRESS = true
	  Log(" ************** CHECKING AND CATCHING A BACK PRESS WHEN A LIST OVERLAY IS ABOVE AN OVERLAY IN THE ACTUAL MODULE")
	  --GC.LISTWIDGET_HOLDER:removeSelf() -- THIS REMOVES THE INFO BELOW THE SMALL LIST
	  if GC.LISTWIDGET_HOLDER then
		GC.LISTWIDGET_HOLDER:dismiss()
...
Runtime:addEventListener( "key", onKeyEventList1 )
	...
	self.onEventCallback = function(event)
		Log("GC.GOT_LIST = "..tostring(GC.GOT_LIST))
		--if GC.GOT_LIST == true then
		if (self) then
