...
-- 
-- 1.1 Changes:
-- * Added ability to fallback to english "en" if key doesn't exist in current language (line 157)
-- 
...
-- Modified to use built-in json library

local NO_STRING = "NO STRING"
...
local loadSettings = function()
	...
	local contents = loadFileContents("language.txt")
...
local loadLanguage = function(name)
	local contents = loadFileContents(name .. ".txt")
...
function new()
	...
	function self:initiate()
		...
		for i=1, #self.settings.languages, 1 do
			...
			if language then
				...
				-- It should add all supported languages not only the last one in the file
				self.supportedLanguages[i] = name
			end
		end
		...
		if self.settings.default then
			...
			print_d("Rosetta: No default language set in language.settings")
		end
	...
	function self:setCurrentLanguage(name)
		...
		if language then
			...
			print_d("Rosetta: Language not found: " .. name or "nil")
		end
	...
	function self:getString(stringName, languageName)
		...
		if languageName then
			...
			if not language then -- if a language name has been passed in but language is null then the language isn't supported
				print_d("Rosetta: Language not supported: " .. languageName)
		...
		if language then
			local label = language[stringName] or self:getLanguage("en")[stringName] or NO_STRING

			if (label == NO_STRING) then
				if (_G.beta == true) then
					print ("Not found: '"..stringName.."'")
				end

				label = string.gsub(string.gsub(stringName, "_", " "), "(%a)([%w_']*)", titleCase)
			end

			return label
		else
			print_d("Rosetta: Current language not set via rosetta:setCurrentLanguage(name)")
		end
